...
class TasksGATEncoder:
    ...
    def _init_pyg_model(self):
        ...
        class PYGATEncoder(nn.Module):
            def __init__(self, input_dim=5, hidden_dim=32, num_layers=3, num_heads=4):
                super().__init__()
    ...
    def _build_edge_index(self, edges, num_nodes):
        ...
        for edge in edges:
            src = edge.get('src', 1)
            dst = edge.get('dst', 1)
            base_weight = edge.get('weight', 1.0)  # 任务图原生weight
            comm_feasibility = edge.get('comm_feasibility', 0.0)  # 通信可行性
            # 确保值在[0,1]区间，避免异常值影响
            fused_weight = min(base_weight * comm_feasibility, 1.0)
            # 兜底：若融合后为0，用base_weight（兼容无comm_feasibility的场景）
            fused_weight = fused_weight if fused_weight > 1e-6 else base_weight
            if 1 <= src <= num_nodes and 1 <= dst <= num_nodes:
                edge_index[0].append(src-1)
                edge_index[1].append(dst-1)
                edge_weights.append(fused_weight)
    ...
    def encode_numpy(self, nodes, edges):
        ...
        for i, node in enumerate(nodes):
            node_features[i] = np.array([
                node.get('cpu_demand', 0) / 100.0,
                node.get('memory_demand', 0) / 128.0,
                node.get('feasible_nodes', 0) / max(num_nodes, 1), # 可行节点数
                node.get('resource_match', 0.0), #  资源匹配度
                node.get('priority', 0.5), # 任务优先级
            ])
...
def test_gat_encoder():
    ...
    encoder = TasksGATEncoder(hidden_dim=32, num_layers=3, num_heads=4, use_pyg=False)
    ...
    try:
        encoder_pyg = TasksGATEncoder(hidden_dim=32, num_layers=3, num_heads=4, use_pyg=True)
