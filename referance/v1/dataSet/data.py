...
class Data:
    ...
    def __init__(self):
        ...
        self.container_service = []  # 微服务和容器的映射（假设这个是事先预定的，即哪个微服务在在哪个容器启用）[0, 1, 2, 3, 4, 5, 6, 7]
        self.container_state_queue = []  # 容器状态队列 3 * ContainerNumber  [-1, cpu, memory, ...]
        ...
        for oneper in dom2.findall('./number/containerNumber'):
            for child in oneper:
                self.container_service.append(int(child.text))
        ...
        self.service_weight = [weight_temp[i: i + self.ServiceNumber] for i in
                               range(0, len(weight_temp), self.ServiceNumber)]
